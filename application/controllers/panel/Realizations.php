...
class Realizations extends CI_Controller {
	...
	public function list($id) {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			...
			$data['rows'] = $this->back_m->get_all_where($this->uri->segment(2), 'realizations_category_id', $id);
			$data['realizations_category'] = $this->back_m->get_one('realizations_categories', $id);
	...
	public function form($type, $realizations_category_id, $id = '') {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
            // DEFAULT DATA
			$data = loadDefaultData();
            ...
            $data['realizations_category'] = $this->back_m->get_one('realizations_categories', $realizations_category_id);
            $data['realizations_categories'] = $this->back_m->get_all('realizations_categories');
	...
	public function action($type, $table, $realizations_category_id, $id = '') {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			$now = date('Y-m-d');
			...
			redirect("panel/$table/list/$realizations_category_id");
		} else {
			redirect('panel');
		}
    }
