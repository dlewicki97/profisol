...
class Jobs_requirements extends CI_Controller {
	...
	public function list($id) {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			...
			$data['rows'] = $this->back_m->get_all_where($this->uri->segment(2), 'job_id', $id);
			$data['job'] = $this->back_m->get_one('jobs', $id);
	...
	public function form($type, $job_id, $id = '') {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
            // DEFAULT DATA
			$data = loadDefaultData();
            ...
            $data['job'] = $this->back_m->get_one('jobs', $job_id);
            $data['jobs'] = $this->back_m->get_all('jobs');
	...
	public function action($type, $table, $job_id, $id = '') {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			$now = date('Y-m-d');
			...
			redirect("panel/$table/list/$job_id");
		} else {
			redirect('panel');
		}
    }
