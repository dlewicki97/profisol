...
class Offers_specials extends CI_Controller {
	...
	public function list($id) {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			...
			$data['rows'] = $this->back_m->get_all_where($this->uri->segment(2), 'offer_id', $id);
			$data['offer'] = $this->back_m->get_one('offers', $id);
	...
	public function form($type, $offer_id, $id = '') {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
            // DEFAULT DATA
			$data = loadDefaultData();
			...
			$data['offer'] = $this->back_m->get_one('offers', $offer_id);
	...
	public function action($type, $table, $offer_id, $id = '') {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			$now = date('Y-m-d');
			...
			redirect('panel/'.$table. "/list/$offer_id");
		} else {
			redirect('panel');
		}
	}
